...
import { dadosGrafico, dadosLinha, dadosEntidades } from '../data/analytics';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          ...
          <p className="text-cyan-200/60 max-w-3xl text-sm sm:text-base">
            Leitura de repercussão externa baseada em consultas temáticas estruturadas. Repercussão na mídia e redes
            sociais, medida por tema como um todo ou por região.
          </p>
        </motion.div>
        ...
        {/* Stacked Bar Chart – Volume Relativo */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          ...
          <GlassCard className="p-4 sm:p-6">
            ...
            <div className="mt-6 pt-4 border-t border-cyan-500/20">
              <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
                {[
                  { color: 'bg-cyan-500/70', label: 'Neutro' },
                  { color: 'bg-emerald-500/70', label: 'Positivo' },
                  { color: 'bg-rose-500/70', label: 'Negativo' },
                ].map(({ color, label }) => (
                  <div key={label} className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded ${color}`} />
                    <span className="text-xs text-cyan-200/70">{label}</span>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-12 sm:mt-16">
            ...
            <GlassCard className="p-4 sm:p-6">
              ...
              <div className="relative h-56 sm:h-72">
                ...
                <svg
                  ...
                  viewBox={`0 0 ${dadosLinha.length - 1} 100`}
                  ...
                  {dadosLinha.map((dado, i) => (
                    <div key={i} className="flex-1 relative h-full">
                      <motion.div
                        ...
                        style={{
                          bottom: `${(dado.saude / maxVolume) * 80}%`,
                          left: '50%',
                      ...
                      <motion.div
                        ...
                        style={{
                          bottom: `${(dado.beneficios / maxVolume) * 80}%`,
                          left: '50%',
                          transform: 'translateX(-50%)',
                        }}
          ...
          {/* Horizontal Bar Chart – Distribuição de Fontes */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            ...
            <GlassCard className="p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                {dadosEntidades.map((entidade, i) => {
                  const total =
                    entidade.instagram + entidade.facebook + entidade.portais + entidade.blogs + entidade.outros;
                  return (
                    <motion.div
                      key={entidade.nome}
                    ...
                    >
                      ...
                      <div className="flex-1 h-6 sm:h-8 flex rounded overflow-hidden">
                        {[
                          { pct: (entidade.instagram / total) * 100, cls: 'bg-pink-500/70', title: `Instagram: ${entidade.instagram}%` },
                          { pct: (entidade.facebook / total) * 100, cls: 'bg-blue-500/70', title: `Facebook: ${entidade.facebook}%` },
                          { pct: (entidade.portais / total) * 100, cls: 'bg-cyan-500/70', title: `Portais: ${entidade.portais}%` },
                          { pct: (entidade.blogs / total) * 100, cls: 'bg-emerald-500/70', title: `Blogs: ${entidade.blogs}%` },
                          { pct: (entidade.outros / total) * 100, cls: 'bg-slate-500/70', title: `Outros: ${entidade.outros}%` },
                        ].map((seg, j) => (
                          <motion.div
                            key={j}
                            initial={{ width: 0 }}
                            ...
                            viewport={{ once: true }}
                            transition={{ duration: 0.6, delay: 0.1 + j * 0.05 + i * 0.1 }}
              ...
              <div className="mt-6 pt-4 border-t border-cyan-500/20">
                <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4">
                  {[
                    { color: 'bg-pink-500/70', label: 'Instagram' },
                    { color: 'bg-blue-500/70', label: 'Facebook' },
                    { color: 'bg-cyan-500/70', label: 'Portais' },
                    { color: 'bg-emerald-500/70', label: 'Blogs' },
                    { color: 'bg-slate-500/70', label: 'Outros' },
                  ].map(({ color, label }) => (
                    <div key={label} className="flex items-center gap-1.5">
                      <div className={`w-2.5 h-2.5 rounded ${color}`} />
                      <span className="text-[10px] sm:text-xs text-cyan-200/70">{label}</span>
                    </div>
                  ))}
                </div>
                <p className="text-center text-[10px] text-cyan-200/40 mt-3">
